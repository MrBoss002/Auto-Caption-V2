...
export async function ensureUser(
  userId: number,
  profile?: Partial<Pick<UserDoc, "firstName" | "lastName" | "username">>
) {
  const db = await getDb();
  const now = new Date();
  await db.collection<UserDoc>("users").updateOne(
    { userId },
    {
      ...
      $setOnInsert: { userId, channels: [], diamonds: 0, createdAt: now },
    },
    { upsert: true }
  );
}
...
export async function getDashboardStats() {
  ...
  return {
    users,
    channels,
    messagesReceived: (statDoc?.messagesReceived as number) ?? 0,
    postsProcessed: (statDoc?.postsProcessed as number) ?? 0,
    postsModified: (statDoc?.postsModified as number) ?? 0,
    stickersSent: (statDoc?.stickersSent as number) ?? 0,
    captionsApplied: (statDoc?.captionsApplied as number) ?? 0,
    buttonsApplied: (statDoc?.buttonsApplied as number) ?? 0,
    commandsReceived: (statDoc?.commandsReceived as number) ?? 0,
    channelsAdded: (statDoc?.channelsAdded as number) ?? 0,
    channelsRemoved: (statDoc?.channelsRemoved as number) ?? 0,
    errors: (statDoc?.errors as number) ?? 0,
