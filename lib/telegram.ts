...
const BRAND = "@MrBossTG";
const SUPPORT_URL = "https://t.me/MrBossSupport";
const UPDATES_URL = "https://t.me/MrBossTG";
...
async function startFlow(ctx: Context) {
  ...
  await replyUser(ctx, `Hey ${htmlEscape(ctx.from.first_name || "there")}\n\nWelcome to <b>Auto-Caption-V2 Bot</b>\n\nYou can use me to manage channels with auto-captioning and button options. Use the buttons below to learn more!\n\nBy ${BRAND}`, {
  ...
  try {
    ...
    if (existing) {
      try {
        ...
        // Previous admin is gone; reclaim channel.
      }
    }
...
bot.on("callback_query:data", async (ctx) => {
  ...
  if (query === "home") {
    await ctx.editMessageText(`Welcome to <b>Auto-Caption-V2 Bot</b>\n\nYou can use me to manage channels with auto-captioning and button options. Use the buttons below to learn more!\n\nBy ${BRAND}`, { parse_mode: "HTML", reply_markup: mainInlineKeyboard() });
    return;
  }
